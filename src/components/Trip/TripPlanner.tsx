...
import { Calendar, MapPin, Users, DollarSign, Download, Share2, Save } from 'lucide-react';
...
export const TripPlanner: React.FC = () => {
  const { currentTrip, updateTrip, isLoading } = useTripStore();
  ...
  if (!currentTrip) {
    return (
      <div className="p-6 text-center">
        <p className="text-gray-600 dark:text-gray-400">No trip selected</p>
      </div>
    );
  }
                      ...
                      <div className="space-y-3">
                        {day.activities.map((activity, actIndex) => (
                          <div key={actIndex} className="flex items-start space-x-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div className="text-sm font-medium text-primary-600 dark:text-primary-400 min-w-0">
                              {activity.timeSlot}
                            </div>
